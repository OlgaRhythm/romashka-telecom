package com.romashka.romashka_telecom.brt.util;
...
public class ContainerLogReader {
    ...
    public String writeDockerLogsToFile(int maxLines) {
        try {
            ...
            if (!completed) {
                process.destroyForcibly();
                log.error("Timeout while writing logs to file");
                return null;
            }
            ...
            log.info("Successfully wrote logs to file: {}", fileName);
            return fileName;
        } catch (Exception e) {
            log.error("Failed to write logs to file", e);
            return null;
        }
    }
} 